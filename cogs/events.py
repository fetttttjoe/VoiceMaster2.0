...
from typing import cast, Optional # Added Optional import
from database.models import Guild, AuditLogEventType, UserSettings # Import necessary models
...
from utils.db_helpers import is_db_value_equal # Import for safe DB value comparison
...
class EventsCog(commands.Cog):
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if before.channel and isinstance(before.channel, discord.VoiceChannel): # Added type guard for Pylance
            ...
            if not is_db_value_equal(channel_before.id, creation_channel_id):
                ...
                logging.info(f"DEBUG: User left creation channel. Not calling _handle_channel_leave.")
        ...
        if after.channel and isinstance(after.channel, discord.VoiceChannel): # Added type guard for Pylance
            ...
            if is_db_value_equal(after.channel.id, creation_channel_id):
                ...
                logging.info(f"DEBUG: User joined non-creation channel. Not calling _handle_channel_creation.")
    ...
    async def _handle_channel_creation(self, member: discord.Member, guild_config: Guild):
        ...
        user_settings: Optional[UserSettings] = await self.voice_channel_service.get_user_settings(member.id)
        ...
        try:
            ...
            logging.error(f"Failed to create temporary channel for user {member.id} in guild {member.guild.id}: {e}", exc_info=True)
            await self.audit_log_service.log_event(
                guild_id=member.guild.id,
